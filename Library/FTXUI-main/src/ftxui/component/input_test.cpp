...
TEST(InputTest, ArrowLeftRight) {
  std::string content = "abcжµ‹жµ‹aжµ‹\naжµ‹\n";
  int cursor_position = 0;
  Component input = Input(&content, {
                                        .cursor_position = &cursor_position,
                                    });
...
TEST(InputTest, Insert) {
  ...
  EXPECT_TRUE(input->OnEvent(Event::Character("жµ‹")));
  EXPECT_EQ(content, "-жµ‹a--bc");

  EXPECT_TRUE(input->OnEvent(Event::Character("a")));
  EXPECT_EQ(content, "-жµ‹aa--bc");

  EXPECT_TRUE(input->OnEvent(Event::Character("жµ‹")));
  EXPECT_EQ(content, "-жµ‹aжµ‹a--bc");
}
...
TEST(InputTest, CtrlArrowLeft2) {
  std::string content = "   word  word  жµ‹ord  woжµ‹d  word   ";
...
TEST(InputTest, TypePassword) {
  ...
  Render(screen, document);
  EXPECT_EQ(screen.PixelAt(0, 0).character, "вЂў");
  EXPECT_EQ(screen.PixelAt(1, 0).character, "вЂў");
}
...
TEST(InputTest, MouseClickComplex) {
  ...
  input->OnEvent(Event::Character("жµ‹"));
  input->OnEvent(Event::Character("иЇ•"));
  input->OnEvent(Event::Character("aвѓ’"));
  input->OnEvent(Event::Character("aМ„"));
  input->OnEvent(Event::Return);
  input->OnEvent(Event::Character("жµ‹"));
  input->OnEvent(Event::Character("иЇ•"));
  input->OnEvent(Event::Character("aвѓ’"));
